...
BaseVolcanoPlot <- function(object,which) {
  ...
  volcano <- create_volcano(object= object,which = which)
...
#'
#' @noRd
PointVolcano <- function(object,which,gene,light = NULL,label_light = T,
                             light_color = "#24ac56",
  ...
  ## basic plot
  p <- BaseVolcanoPlot(object,which = which)
  ...
  if (!is.null(gene)) {
    ## gene label data
    gene_label <- genePoint(object = object,which = which,gene = gene)
    ...
    p <- p +
      ...
      geom_volcano_text(data = label_up,
                        ...
                        nudge_x =  volcano_nudge_x_up(object = object, label_data = label_up,which = which)) +
      geom_volcano_text(data = label_down,
                        mapping = aes(label = rn,color = point.color),
                        nudge_y      = -4,
                        hjust        = 1,
                        min.segment.length = 0,
                        nudge_x =  volcano_nudge_x_down(object = object, label_data = label_down,which = which))
  }

  if (!is.null(light)) {
    ## highlight data
    light_label <- genePoint(object = object,which = which,gene = light)
...
volcano_nudge_x_down <- function(object,label_data,which,just = 0) {
